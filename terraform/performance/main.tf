...
resource "null_resource" "install_binaries" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "aws s3 cp s3://${var.s3_bucket}/integration-test/binary/${var.cwa_github_sha}/linux/${var.arc}/${local.ami_family["install_package"]} .",
      "aws s3 cp s3://${var.s3_bucket}/integration-test/validator/${var.cwa_github_sha}/linux/${var.arc}/${local.install_validator} .",
...
#####################################################################
# Setup mock cloudwatch server 
#####################################################################
data "template_file" "mock_server" {
  template = file("../../localstack/docker-compose.yml")
}
...
resource "null_resource" "setup_mock_server" {
  ...
  provisioner "file" {
    source      = data.template_file.mock_server.rendered
    destination = "${local.temp_directory}/docker-compose.yml"
  }

  provisioner "remote-exec" {
    inline = [
      "sudo docker-compose -f ${local.temp_directory}/docker-compose.yml up -d",
      "curl localhost:4566",
      "exit 1"
    ]
  }
...
resource "null_resource" "validator_linux" {
  count      = var.family != "windows" ? 1 : 0
  depends_on = [aws_instance.cwagent, null_resource.upload-validator, null_resource.install_binaries, null_resource.setup_mock_server]
